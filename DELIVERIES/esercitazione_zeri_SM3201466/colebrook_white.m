...
fplot(f_zero); hold on; fplot(@(x) 0 .* x, Color='Black'); xlim([-1, 1]); ylim([-1,1]); hold off; % plots graph of 0 for a good idea
title("Function to Calculate Zero of")

input("Press enter to continue with the next graph\n> ")

fplot(g); hold on; fplot(@(x) x, Color='Black'); xlim([-1, 1]); ylim([-1,1]); hold off; % same but with fixed point
