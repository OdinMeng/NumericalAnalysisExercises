...
semilogy(1:i_1, abs(sc1), Color='red', Marker='*'); hold on;
semilogy(1:i_2, abs(sc2), Color='blue', Marker='hexagram'); 
...
fplot(@(x) 1+atan(x)); hold on; fplot(@(x) x, Color='Black'); hold off;
input("Press enter to continue\n> ")

% graficamente si evince che un intervallo opportuno è [2.1, 2.15] 
g = @(x) 1+atan(x);

[x,i_, sc] = pfisso(g, 2.1, 1e-12, 1000);
...
% create graph
semilogy(1:i_, abs(sc), Color='red', Marker='*');
